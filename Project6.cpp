 ...
 */
int cmp2int(int x[], int n){
    if(n == -1)
    {
        return x[n+1];
    }
    if(x[n] < cmp2int(x, n-1))
    {
        return x[n];
    }
    else
    {
        return cmp2int(x, n-1);
    }
}
int minRec1(int x[], int n) {
    int min = x[n-1];
    return cmp2int(x, n-2);
}
...
int minRec2(int x[], int n) {
    ...
    else
    {
        if(n % 2 == 1)
        {
            int num1 = minRec2(x, n/2);
            int num2 = minRec2(&x[n/2], n/2 + 1);
            if(num1 < num2)
            {
                return num1;
            }
...
double sqrtRec(double x, double low_guess, double high_guess) {
    ...
    double average = (low_guess + high_guess) / 2;
    if(low_guess * low_guess == x)
    {
        return low_guess;
    }
    else if(high_guess * high_guess == x)
    {
        return high_guess;
    }
    ...
    {
        return sqrtRec(x, low_guess, average);
    }
    else if(average * average < x)
    {
        return sqrtRec(x, average, high_guess);
    }
}
...
int solveMazeRec(int row, int col) {
    ...
    if(maze[row][col-1] == 0)
    {
        left = solveMazeRec(row,col-1);
    }
    if(maze[row][col+1] == 0)
    {
        right = solveMazeRec(row,col+1);
    }
    if(maze[row+1][col] == 0)
    {
        down = solveMazeRec(row+1, col);
    }
    if(left + right + down + up== 0)
    {
        maze[row][col] = 0;
        return 0;
    }
...
Martian change(int cents) {
    Martian dodeks = {0,0,0};
    Martian nicks = {0,0,0};
    Martian pennies = {0,0,0};
    if(cents <= 0)
    {
        return dodeks;
    }
    if(cents >= 12)
    {
        dodeks = change(cents - 12);
        dodeks.dodeks += 1;
    }
    if(cents >= 5)
    {
        nicks = change(cents - 5);
        nicks.nicks += 1;
    }
    if(cents >= 1)
    {
        pennies = change(cents - 1);
        pennies.pennies += 1;
    }
    int pennysum = pennies.dodeks + pennies.nicks + pennies.pennies;
    int dodeksum = dodeks.dodeks + dodeks.nicks + dodeks.pennies;
    int nicksum = nicks.dodeks + nicks.nicks + nicks.pennies;
    int max = pennysum;
    if(dodeksum != 0 && dodeksum < max)
    {
        max = dodeksum;
    }
    if(nicksum != 0 && nicksum < max)
    {
        max = nicksum;
    }
    if(max == pennysum)
...
Martian change(int cents, int nick_val, int dodek_val) {
    ...
    if(cents >= dodek_val)
    {
        dodeks = change(cents - dodek_val, nick_val, dodek_val);
        dodeks.dodeks += 1;
    }
    if(cents >= nick_val)
    {
        nicks = change(cents - nick_val, nick_val, dodek_val);
        nicks.nicks += 1;
    }
    if(cents >= 1)
    {
        pennies = change(cents - 1, nick_val, dodek_val);
        pennies.pennies += 1;
    }
    int pennysum = pennies.dodeks + pennies.nicks + pennies.pennies;
    int dodeksum = dodeks.dodeks + dodeks.nicks + dodeks.pennies;
